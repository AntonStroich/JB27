	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (sc.hasNextLine()) {
				...
				// max is empty by default and it will be overwritten by any new string which
				// length more than current.
				if (str.length() > max.length()) {
					max = str;
					// compare length of the next row with length of the min variable;
					// min is not empty by default because it was set up at the first loop and it
					// will be overwritten by any new string which length more than current.
				} else if (str.length() < min.length()) {
					min = str;
				}
			}
		}
		// return values of max and min variables and their length via the length()
		// method
